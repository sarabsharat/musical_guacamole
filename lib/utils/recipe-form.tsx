...
export function IngredientMapperGrid({ rows, references, onUpdate, onRemove, onAdd }: IngredientGridProps) {
    ...
    <div className="space-y-4">
        {rows.map((row) => (
            ...
            <button type="button" onClick={() => onRemove(row.keyId)} className="absolute top-2 right-2 border border-black bg-red-100 hover:bg-red-500 hover:text-white px-1 font-mono text-[10px] font-bold rounded-none">
        X
        </button>
        <div className="md:col-span-3">
    <div className="text-[10px] font-mono uppercase text-neutral-400">Extracted Text / Name</div>
    <input
    required
    type="text"
    value={row.rawText}
    onChange={(e) => onUpdate(row.keyId, "rawText", e.target.value)}
    className="w-full border border-black p-1.5 font-mono text-xs rounded-none focus:outline-none mt-1 bg-neutral-50"
        />
        </div>
    ...
    className="w-full border border-black p-1.5 font-mono text-xs rounded-none focus:outline-none mt-1"
        ...
        <div className="md:col-span-2">
    <div className="text-[10px] font-mono uppercase text-neutral-400">Weight (g)</div>
        <input
    required
    type="number"
    min={0.1}
